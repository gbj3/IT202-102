...
function getDB(){
    ...
    if(!isset($db)) {
        try{
            ...
            //Updated so we don't leak our DB password when an exception occurs
            //Note, it still incorrectly hits the php logs, but at least it's not shown to the user
            error_log("getDB error: " . var_export($e, true));
            flash("There was a database connection error, please try again", "warning");
            $db = null;
        }
    }
    return $db;
